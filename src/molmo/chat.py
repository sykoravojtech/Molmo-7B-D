...
def chat_loop(model: AutoModelForCausalLM, processor: AutoProcessor, image: Image.Image) -> None:
    """Interactive chat loop with the model."""
    print("=" * 60)
    print("Molmo Chat Interface")
    print("=" * 60)
    print(f"Image loaded successfully!")
    print("Type your questions or commands. Type 'quit', 'exit', or 'q' to end the session.")
...
def get_vlm_prompt() -> str:
    ...
    return '''You are a specialised circuit-diagram interpreter.
Given the image of an electrical circuit diagram, identify which symbol classes from the list below appear in the image.
List only the classes you find, in a JSON array, and nothing else.
...
Given the image of an electrical circuit diagram, identify all symbols of class "{cls_name}".
For each symbol from that class, output one <points> tag and nothing else, in the exact format:
<points x1="…" y1="…" x2="…" y2="…" alt="{cls_name}">{cls_name}</points>
Use the coordinates of the bounding polygon of each symbol. Do not include any additional text or tags.

Example:
<points x1="20.8" y1="42.4" x2="21.0" y2="53.6" alt="{cls_name}">{cls_name}</points>'''
