...
def evaluate_molmo_predictions(
    molmo_response: str, xml_path: str, image_width: int, image_height: int
) -> Dict:
    """
    ...
    for component_type, coordinates in molmo_predictions:
        ...
        if gt_component_type in gt_annotations:
            ...
            results["unmatched_predictions"].extend(
                [(component_type, coord) for coord in processed_coords]
            )

        component_results["accuracy"] = (
            component_results["correct"] / component_results["total"]
...
def print_evaluation_report(results: Dict, debug: bool = False) -> None:
    ...
    print(
        f"Incorrect Predictions: {results['total_predictions'] - results['correct_predictions']}"
    )
    print()
    ...
    for component_type, comp_results in results["by_component"].items():
        ...
        if comp_results["incorrect_coords"]:
            print(
                f"  Incorrect coordinates: {comp_results['incorrect_coords'][:3]}{'...' if len(comp_results['incorrect_coords']) > 3 else ''}"
            )
        print()
...
def debug_coordinate_comparison(
    molmo_response: str, xml_path: str, image_width: int, image_height: int
) -> None:
    ...
    for component_type, coordinates in molmo_predictions:
        ...
        abs_coords = [
            (x * image_width / 100.0, y * image_height / 100.0) for x, y in coordinates
        ]
    ...
    for component_type, coordinates in molmo_predictions:
        ...
        for x, y in coordinates:
            plt.scatter(
                ...
                s=100,
                edgecolor="black",
                label=f"Predicted {component_type}",
            )
...
def visualize_predictions_to_file(
    image_path: str, molmo_response: str, xml_path: str, output_path: str = None
) -> str:
    """
    ...
    for obj_name in gt_annotations.keys():
        ...
        legend_elements.append(
            patches.Patch(
                color=color, label=f"GT {obj_name} ({len(gt_annotations[obj_name])})"
            )
        )
    ...
    for component_type, color in used_colors:
        coord_count = len(
            [
                coord
                for ct, coords in molmo_predictions
...
if __name__ == "__main__":
    ...
    results = evaluate_molmo_predictions(
        example_molmo_response, xml_path, image_width, image_height
    )
